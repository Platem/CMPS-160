...
function main() {
	...
	// Texture Mapping toggler
	document.getElementById('bTexture').addEventListener('click', function(e) {
		e.preventDefault();
		draw_options.useTexture ? draw_options.useTexture = false : draw_options.useTexture = true;
		document.getElementById('circle-tex').classList.toggle('active');
		draw();
	});

	document.getElementById('bmTexture').addEventListener('click', function(e) {
		e.preventDefault();
		draw_options.mixTexColors ? draw_options.mixTexColors = false : draw_options.mixTexColors = true;
		draw_options.mixTexColors ? gl.uniform1i(u_mixTexColors, 1.0) : gl.uniform1i(u_mixTexColors, 0.0);
		document.getElementById('circle-mtex').classList.toggle('active');
		draw();
	});

	document.getElementById('bTextureT').addEventListener('click', function(e) {
		e.preventDefault();
		toggleTexture();
		draw();
	});
	...
	$(document).on('click', '.removeList', function(e) {
		...
		if (picked_object > index) picked_object--;
...
function drawPolygon(points, color) {
	...
	for (let i = 0; i < points.length; i++) {
		...
		v.push((points[i][0] % 500) / 500);
		v.push((points[i][1] % 500) / 500);
	}

	// Write vertices into buffer
	gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(v), gl.STATIC_DRAW);

	gl.uniform1f(u_useTex, 0.0);

	if (draw_options.opacity_enabled) {
		// Draw points
		gl.drawArrays(gl.TRIANGLE_FAN, 0, v.length / 9);
	} else {
		// Draw points
		gl.drawArrays(gl.TRIANGLE_FAN, 0, v.length / 8);
	}
}
