...
var Obj = function() {
	...
	this.drawLine = function(m_point) {
		...
		if (draw_options.opacity_enabled) {
			gl.drawArrays(gl.POINTS, 0, vertices.length / 9);
		} else {
		...
		if (draw_options.opacity_enabled) {
			// Draw lines
			gl.drawArrays(gl.LINE_STRIP, 0, vertices.length / 9);
		} else {
			// Draw lines
			gl.drawArrays(gl.LINE_STRIP, 0, vertices.length / 8);
		}
	};
		...
		if (withID) {
			...
			if (draw_options.draw_normals) {
				for (let i = 0; i < this.polygons.length; i++) {
					for (let j = 0; j < this.polygons[i].elements.length; j++) {
						...
						gl.uniform1f(u_useTex, 0.0);
						if (draw_options.opacity_enabled) {
							gl.drawArrays(gl.LINES, 0, vertices.length / 9);
						} else {
							// Draw lines
			...
			if (draw_options.draw_surfaces) {
				for (let i = 0; i < this.polygons.length; i++) {
					...
					for (let j = 0; j < this.polygons[i].elements.length; j++) {
						...
						// Calc light
						let I = [0.0, 0.0, 0.0], Ia = [0.0, 0.0, 0.0], Id = [0.0, 0.0, 0.0], Is = [0.0, 0.0, 0.0];
						let normal = vertex.n;
						...
						for (let j = 0; j < lights.length; j++) {
							if (lights[j].enabled) {
								...
								if (draw_options.light_difuse) {
									...
									Id = [draw_options.surface_kd[0] * lights[j].color[0] * S,
								...
								if (draw_options.light_specular) {
									...
									Is = [draw_options.surface_ks[0] * lights[j].color[0] * S,
						...
						if (draw_options.useTexture && !draw_options.mixTexColors) {
							// only specular
							v.push(Ia[0] + Is[0]);
							v.push(Ia[1] + Is[1]);
							v.push(Ia[2] + Is[2]);
						...
						v.push(Math.abs(vertex.x) / 500);
						v.push(Math.abs(vertex.y) / 500);
					}

					// Write vertices into buffer
					gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(v), gl.STATIC_DRAW);

					if (draw_options.useTexture) {
						gl.uniform1f(u_useTex, 1.0);
					} else {
						gl.uniform1f(u_useTex, 0.0);
					}
					if (draw_options.opacity_enabled) {
			...
			if (draw_options.draw_skeleton) {
				for (let i = 0; i < this.polygons.length; i++) {
					...
					if (draw_options.opacity_enabled) {
						// Draw points
						gl.drawArrays(gl.LINE_LOOP, 0, v.length / 9);
					} else {
						// Draw points
						gl.drawArrays(gl.LINE_LOOP, 0, v.length / 8);
					}
				}
			}
			...
			if (draw_options.draw_points) {
				...
				if (draw_options.opacity_enabled) {
					// Draw points
					gl.drawArrays(gl.POINTS, 0, v.length / 9);
				} else {
					// Draw points
