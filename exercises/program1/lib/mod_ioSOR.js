...
function setupIOSOR(elementName) {
	...
	document.getElementById(elementName).addEventListener('change', readEvent2);
	setupComplete = true;
}
...
/*
 * Edit by Keivn Romero Peces-Barba
 *
 * Description:
 * 		- Possible modifications for next version: support for multiple objects in one file.
 * 		- Objects are not always conected so adding option to support various objects seems cool to me.
 * 		- BTW most of the code is pretty similar.
 *
 * Notes:
 * 		- I've notice some checks should be done to prevent trying to load bad formated files (check if there's an active object when firs character is v or f)
 */

function readEvent2(_e) {
	let f = _e.target.files[0];
	...
	if (f) {
		...
		r.onload = function(e) {
			...
			while (1) {
				...
				if (line.charAt(0) == 'f') {
					line = line.substr(2);
					SORCollection[active].indexes.push(parseFloat(line.substr(0, line.indexOf(" "))));

					line = line.substr(line.indexOf(" ") + 1);
					SORCollection[active].indexes.push(parseFloat(line.substr(0, line.indexOf(" "))));

					line = line.substr(line.indexOf(" ") + 1);
					SORCollection[active].indexes.push(parseFloat(line));
				} else if (line.charAt(0) == 'v') {
					line = line.substr(2);
					SORCollection[active].vertices.push(parseFloat(line.substr(0, line.indexOf(" "))));

					line = line.substr(line.indexOf(" ") + 1);
					SORCollection[active].vertices.push(parseFloat(line.substr(0, line.indexOf(" "))));

					line = line.substr(line.indexOf(" ") + 1);
					SORCollection[active].vertices.push(parseFloat(line));
				} else if (line.charAt(0) == 'o') {
					SORCollection.push(new SOR(line.substr(line.indexOf(" ") + 1), [], []));
					active++;
				}
			...
			console.log("Extracted");
			console.log(SORCollection);
			extractedSORs = SORCollection;
		}
		r.readAsText(f);
	} else {
		alert("Failed to load file");
...
function readFile2() {
	if (extractedSORs != null && setupComplete == true) {
		alert('Loading collection. It contains ' + extractedSORs.length + ' object(s).');
...
function saveFile2(SORCollection, name) {
	let text = '';
	...
	for (let i = 0; i < SORCollection.length; i++) {
		let SOR = SORCollection[i];

		text += "o " + SOR.objName + "\n";

		// Convert Vertices to Text
		for (let i = 0; i < SOR.vertices.length; i = i + 3) {
			text = text + "v " + SOR.vertices[i] + " " + SOR.vertices[i + 1] + " " + SOR.vertices[i + 2] + "\n";
		}

		// Convert Indexes to Text
		for (let i = 0; i < SOR.indexes.length; i = i + 3) {
			text = text + "f " + SOR.indexes[i] + " " + SOR.indexes[i + 1] + " " + SOR.indexes[i + 2] + "\n";
		}
	}
	console.log(text);
